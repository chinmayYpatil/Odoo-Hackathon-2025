...
export default function TagSelector({ selectedTags, onTagsChange, className = '' }: TagSelectorProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="border border-gray-300 rounded-md p-2 min-h-[42px] flex flex-wrap gap-2 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
          ...
          {filteredTags.slice(0, 10).map((tag) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span>{tag.name}</span>
                <span className="text-xs text-gray-500">{tag.question_count} questions</span>
              </div>
            </button>
          ))}
        </div>
